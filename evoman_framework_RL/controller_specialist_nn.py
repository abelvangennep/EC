...
population_size = 10
for en in range(1, 2):
    results = np.zeros(((number_generations+1)*population_size, 8)) #Generation, Individual, Parents, Species, Fitness, time, avg.gen fitness, avg.gen dist
    ...
    for i in range(number_generations): #number of generations
        ...
        for pcont in pop:
            ...
            results[gen * population_size + pcont.get_id(), 4] = vfitness
            results[gen * population_size + pcont.get_id(), 5] = time.time()-start_ind

        overview[gen,0] = sum(fitnesses)/len(fitnesses)
        overview[gen,1] = calc_avg_dist(pop)
        results[gen*population_size,6] = sum(fitnesses)/len(fitnesses)
        results[gen * population_size, 7] = calc_avg_dist(pop)
        ...
        for m in range(len(children)):
            id_node, highest_innov_id = mutate(children[m], id_node, highest_innov_id)
            children[m].set_id(m)
        ...
        gen+=1
        #repeat loop
    results_df = pd.DataFrame(results, columns = ['Generation', 'Individual', 'Parents', 'Species', 'Fitness', 'Time elapsed', 'Avg. fitness', 'Avg. distance'])
