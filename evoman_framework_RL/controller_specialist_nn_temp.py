...
def neat_optimizer(number_generations, population_size, mutation_prob, compat_threshold,link_insert_prob,node_insert_prob, enemy):
    for en in enemy:
        overview = np.zeros((number_generations,2))
        # Update the enemy
        env.update_parameter('enemies', [en])
...
def neat_iterations(parameters):
    ...
    for _ in range(num_iterations):
        best_fitnesses.append(neat_optimizer(number_generations, population_size, mutation_prob,compat_threshold,
...
space = hp.choice('Type_of_model',[{
        ...
        'mutation_prob': hp.uniform("population_size_3", 0, 1),
        'compat_threshold': hp.uniform("population_size_3", 1, 12),
        'link_insert_prob': hp.uniform("population_size_3", 0, 1),
        'node_insert_prob': hp.uniform("population_size_3", 0, 1),
            }])
